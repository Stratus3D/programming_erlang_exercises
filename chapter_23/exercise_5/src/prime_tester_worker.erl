...
%% API
-export([start_link/0, is_prime/2, stop/1]).
...
is_prime(Pid, Number) ->
    gen_server:cast(Pid, {is_prime, Number}).
...
%% @private
%%--------------------------------------------------------------------
handle_cast({is_prime, Number}, State) ->
    % Invoke lib_primes:is_prime to determine if the number is prime or not
    Result = lib_primes:is_prime(Number),
    % Send the results back to the prime_tester_load_balancer
    ok = gen_server:cast(prime_tester_load_balancer, {worker_result, self(), Result}),
    {noreply, State};
